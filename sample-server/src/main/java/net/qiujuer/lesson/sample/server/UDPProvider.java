        ...
        public void run() {
            ...
            try {
                ...
                while (!done) {
                    ...
                    // 解析命令与回送端口
                    // message is sent byte by byte, so if we have byte1 byte2, we need to do byte calcul to recompose as byte2 byte1 to get original message
                    int index = UDPConstants.HEADER.length;
                    // short has 2 bytes
                    short cmd = (short) ((clientData[index++] << 8) | (clientData[index++] & 0xff)); // oxff will fill zero
                    // int has 4 bytes
                    ...
                    if (cmd == 1 && responsePort > 0) {
                        // 构建一份回送数据 -> tell client where server's ip and port
